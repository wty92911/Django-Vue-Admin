<template>
  <d2-container :class="{ 'page-compact': crud.pageOptions.compact }">
    ...
    >
      ...
      <template slot="partsSlot" slot-scope="scope">
        <i class="d2-mb-5" 
          v-for="(item,index) in scope.row.parts" :key="index" 
        > {{ item.vehicle_part.name }} {{ item.quantity }} 个 共 {{ item.total_price }} 元<br>
        </i>
      </template>
    </d2-crud-x>
  </d2-container>
